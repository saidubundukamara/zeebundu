    ...
    switch (sectionType) {
      case 'hero':
        ...
        if (!content.backgroundImage && !content.backgroundVideo) {
          warnings.push('Consider adding a background image or video for visual impact');
        }
        break;
      ...
      case 'gallery':
        ...
        if (content.images.length === 0) {
          warnings.push('Gallery is empty - add some images to showcase your business');
        }
        if (content.images.length > 0) {
          const imagesWithoutAlt = content.images.filter((img: any) => !img.alt || !img.media.alt);
          if (imagesWithoutAlt.length > 0) {
            warnings.push(`${imagesWithoutAlt.length} images missing alt text for accessibility`);
          }
        }
        break;
      ...
      case 'about':
        ...
        if (content.description.length < 100) {
          warnings.push('About description is quite short - consider adding more detail');
        }
        break;
...
export function validateTemplateContent(
...
): { isValid: boolean; errors: string[]; warnings: string[]; requiredSections: string[] } {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  const requiredSections = templateRequirements[templateType] || templateRequirements.default;
  ...
  sectionValidations.forEach(validation => {
    if (!validation.isValid) {
      errors.push(...validation.errors.map(err => `${validation.type}: ${err}`));
    }
    warnings.push(...validation.warnings.map(warning => `${validation.type}: ${warning}`));
  });

  // Template-specific recommendations
  if (templateType === 'hotel' && !activeSections.includes('gallery')) {
    warnings.push('Hotels should showcase their facilities with a gallery section');
  }

  if (templateType === 'gas-station' && !activeSections.includes('services')) {
    warnings.push('Gas stations should highlight their fuel types and services');
  }
...
export function analyzeSEOContent(sections: Array<{ type: string; content: any; isActive: boolean }>) {
  ...
  if (heroSection) {
    ...
    if (!heroSection.content.title || heroSection.content.title.length < 30) {
      recommendations.push('Hero title should be at least 30 characters for better SEO');
      seoScore -= 10;
    }
    if (heroSection.content.title && heroSection.content.title.length > 60) {
      recommendations.push('Hero title is too long for search engine snippets (>60 chars)');
      seoScore -= 5;
    }

    // Description length check
    if (!heroSection.content.description || heroSection.content.description.length < 120) {
      recommendations.push('Hero description should be at least 120 characters for better SEO');
      seoScore -= 10;
    }
  } else {
  ...
  const gallerySection = sections.find(s => s.type === 'gallery' && s.isActive);
  if (gallerySection && gallerySection.content.images) {
    ...
    if (imagesWithoutAlt.length > 0) {
      recommendations.push(`${imagesWithoutAlt.length} images missing alt text for SEO and accessibility`);
      seoScore -= Math.min(15, imagesWithoutAlt.length * 3);
    }
  }

  // Check for contact information
  const contactSection = sections.find(s => s.type === 'contact' && s.isActive);
  if (!contactSection) {
