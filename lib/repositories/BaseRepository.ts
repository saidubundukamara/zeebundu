...
export abstract class BaseRepository<T extends Document> implements IBaseRepository<T> {
  ...
  protected async getCollection(): Promise<Collection<T>> {
    if (!this.collection) {
      this.collection = await getCollection(this.collectionName) as unknown as Collection<T>;
    }
  ...
  async updateById(id: string | ObjectId, update: UpdateOperation): Promise<T | null> {
    try {
      const collection = await this.getCollection();
      const objectId = typeof id === 'string' ? new ObjectId(id) : id;
  ...
  async updateOne(query: DatabaseQuery, update: UpdateOperation): Promise<T | null> {
    try {
      const collection = await this.getCollection();
      const { filter = {} } = query;
  ...
  async updateMany(query: DatabaseQuery, update: UpdateOperation): Promise<number> {
    try {
      const collection = await this.getCollection();
      const { filter = {} } = query;
      ...
      const result = await collection.updateMany(filter as any, updateDoc as any);
  ...
  protected handleError(error: any, operation: string): Error {
    console.error(`Database error in ${this.collectionName}.${operation}:`, error);
    
    if (error instanceof MongoError) {
      if (error.code === 11000) {
        return new Error('Duplicate key error: A document with this key already exists');
      }
    }
