...
export class MediaRepository extends BaseRepository<MediaAsset> {
  ...
  async findByBusinessId(businessId: string | ObjectId): Promise<MediaAsset[]> {
    const id =
      typeof businessId === "string" ? new ObjectId(businessId) : businessId;
  ...
  async getMediaByFilters(filters: MediaFilters): Promise<MediaAsset[]> {
    ...
    if (filters.businessId) {
      const id =
        typeof filters.businessId === "string"
          ? new ObjectId(filters.businessId)
          : filters.businessId;
      filter.businessId = id;
    }
    ...
    if (filters.uploadedAfter || filters.uploadedBefore) {
      ...
      if (filters.uploadedBefore)
        filter.createdAt.$lte = filters.uploadedBefore;
    }
  ...
  async getPopularTags(
    ...
    return (await collection.aggregate(pipeline).toArray()) as Array<{
      tag: string;
      count: number;
    }>;
  }
